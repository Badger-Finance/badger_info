...
export async function fetchCycles(page: number) {
  try {
    const result = await fetch(`${ANALYTICS_API_URL}/cycles?chain=polygon&limit=5&offset=${page * 5}`)
...
export async function fetchCycle(cycleNumber: number) {
  try {
    const result = await fetch(`${ANALYTICS_API_URL}/cycle/${cycleNumber}/?chain=polygon`)
    const json = await result.json()
    console.log(json)
    return {
      error: false,
